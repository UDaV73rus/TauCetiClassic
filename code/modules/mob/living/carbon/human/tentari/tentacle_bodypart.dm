...
// Destroy released tentacle and refs to bodypart
...
// Break of released tentacle, destroy everything after break
...
// Stop actions and clear refs to destroyed grabber
...
/datum/bodypart_controller/tentacle/proc/start_retrieving_grabber()
	stop_drag_owner()
	if(retrieving_state == FALSE) // Prevent multiple calls of retrieving_grabber()
...
// Process of stepmoving grabber to owner
...
// Try to retrieve nearby grabber and destroy it
...
// Retrieve tentacle bodies and destroy grabber
...
// Process of stepmoving owner to grabber
...
/datum/bodypart_controller/tentacle/proc/adjust_limit(var/value)
	to_chat(world, "adjust_limit([value])")
